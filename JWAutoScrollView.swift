    ...
    func reloadData() {
        ...
        if let d =  delegate{
            if !isAutoScroll {
                ...
                bgScrollView?.contentSize = CGSize(width:CGFloat(1 * pageCount)*(bgScrollView?.bounds.width)!, height: (bgScrollView?.bounds.height)!)
                bgScrollView?.contentOffset = CGPoint(x: 0, y: 0)
...
//                    }
                    ...
                    if let d =  delegate {
                        if (tempIndex) < pageCount {
                            d.autoScrollView(autoScrollView: self, index: tempIndex, imageButton: view)
                            //设置当前索引
                            view.tag = tempIndex
                        }
                    }
        ...
        //new add
        if isHiddenBottom {
            self.pageControl?.isHidden = isHiddenBottom
        }
    }
