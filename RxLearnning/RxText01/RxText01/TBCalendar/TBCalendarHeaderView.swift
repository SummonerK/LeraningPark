...
class TBCalendarHeaderView: UIView {
    ...
    lazy var topView: UIView = self.getTopView()
    lazy var dateLable: UILabel = self.getDateLable()
    lazy var previousButton: UIButton = self.getPreviousButton()
    lazy var nextButton: UIButton = self.getNextButton()
    ...
    func createAttrStr(monthStr: String, yearStr: String) -> NSMutableAttributedString{
        let monthDict = [NSFontAttributeName: UIFont.systemFont(ofSize: 24), NSForegroundColorAttributeName:UIColor.UIColorFromRGB(rgb: 0x4F505F)]
        let yearDict = [NSFontAttributeName: UIFont.systemFont(ofSize: 14), NSForegroundColorAttributeName:UIColor.UIColorFromRGB(rgb: 0x999999)]
        ...
        mutableAtt.addAttributes(yearDict, range: NSRange.init(location: monthStr.length, length: yearStr.length))
