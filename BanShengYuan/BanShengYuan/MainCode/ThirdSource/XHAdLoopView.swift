...
class XHAdLoopView: UIView {
    ...
    fileprivate func createPageView(){
        ...
        pageControl = UIPageControl(frame: CGRect(x: pageX/2, y: pageY, width: pageW, height: pageH))
    ...
    //图片点击
    func imageViewClick(){
        if self.delegate != nil && (self.delegate?.responds(to: #selector(XHAdLoopViewDelegate.adLoopView(_:IconClick:)))) != nil {
